...
NEXT_PUBLIC_GENLAYER_CONTRACT_ADDRESS in your environment once the contract is deployed - nothing here is faked or mocked in the meantime.
```
...
the pattern is a legitimate one-time hydration/fetch-kickoff, not an accidental render loop - see `lib/walletContext.tsx`, `lib/useTransactionTracker.ts`, both list pages, and the
